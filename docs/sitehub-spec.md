...
- **Galleries (§8) and Forms (§9) are features #1 and #2.** The FRAN engagement (SoW 2026-07-13) added **#3 `partners`** (partner organizations & program listings, staff-managed) and **#4 `events`** (outreach events & classes; informational in FRAN Phase 1, Need Navigator registration in FRAN Phase 2). Future candidates: slides/hero editing, blog — built only against concrete client demand.
- **Per-site multilingual content** (moved out of non-goals for FRAN, which launches bilingual EN/ES): a site may declare additional locales in settings. Pages-as-code localize via per-locale templates (`pages/es/...` served under an `/es` URL prefix, default locale at root); feature entities (partners, events) localize via translatable jsonb fields edited side-by-side in the admin. Language toggle in the site layout. Only enabled sites carry any of this.
- **Platform-layer concerns are NOT features and are never per-site:** accessibility baseline **plus the user-facing accessibility toolbar (high-contrast mode, adjustable text size, link highlighting, legible-font option, large cursor — promised in the FRAN SoW and shipped uniformly to every site)**, cookie consent / privacy compliance, security headers, asset deployment/caching structure, sitemap/robots. Uniform everywhere, always on, one implementation — for sanity.
...
Out of scope for SiteHub, deliberately: e-commerce (the legacy cart/checkout/Stripe/Authorize.Net/promo-code/shipping/tax stack is vestigial — no migrating site sells online), Algolia/Scout product search, blog/publications + RSS feed, the *legacy* events module (a modern `events` feature exists per §5a — built for FRAN, not ported), staff biographies, the dynamic form builder (fields/conditional logic/submission spreadsheets), Constant Contact, content scheduling (`contentschedules` recurrence), per-page role gating, inline on-page editing, the visual layout composer (DB-stored Tailwind classes), dashboard widgets, the policies module (legal pages flatten like any other page), menus-as-data, payments of any kind, public user accounts, theme-editing UI. (Multi-language moved to §5a as a per-site capability, 2026-07-13, driven by the FRAN engagement.) Do not add packages beyond §2 without approval.
